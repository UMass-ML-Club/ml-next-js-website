...
export default function Main() {
  return (
    <main className="bg-umass-neutral-light p-2 dark:bg-umass-neutral-dark">
      <div className="flex items-center justify-center p-4 bg-slate-400 dark:bg-black mx--10">
      ...
      <div className="m-2 grid grid-rows-3 md:grid-rows-1 md:grid-cols-3">
        {mainAnimationData.map((item, index) => (
          <a className="hover:bg-black hover:text-white transition duration-0 hover:duration-500" href={item.url} key={index}>
            <div className="m-2 flex flex-col items-center" key={index}>
              ...
              <b><p className="flex items-center justify-center text-4xl">{item.description}</p></b>
            </div>
          </a>
        ))}
      </div>
